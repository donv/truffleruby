    ...
    patches: {
      'cparse.c' => [
        ...
        { # parse_main
          ...
          replacement: 'VALUE tr_tmp = rb_hash_aref(v->token_table, tok == 0 ? Qfalse : rb_tr_managed_from_handle(tok));'
        },
        { # parse_main
          match: 'if (NIL_P(v->t)) {',
        ...
        { # parse_main
          match: /rb_hash_aref\((.+?), tok\)/,
          replacement: 'rb_hash_aref(\\1, tok == 0 ? Qfalse : tok)'
        },
        { # extract_user_token
          match: '*tok = Qfalse;',
          replacement: '*tok = INT2FIX(0);'
        },
        { # extract_user_token
